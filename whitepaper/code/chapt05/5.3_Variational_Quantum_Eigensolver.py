...
# 
# [![Download Notebook](https://mindspore-website.obs.cn-north-4.myhuaweicloud.com/website-images/r2.2/resource/_static/logo_notebook_en.svg)](https://mindspore-website.obs.cn-north-4.myhuaweicloud.com/notebook/r2.2/mindquantum/en/case_library/mindspore_vqe_for_quantum_chemistry.ipynb)&emsp;
# [![Download Code](https://mindspore-website.obs.cn-north-4.myhuaweicloud.com/website-images/r2.2/resource/_static/logo_download_code_en.svg)](https://mindspore-website.obs.cn-north-4.myhuaweicloud.com/notebook/r2.2/mindquantum/en/case_library/mindspore_vqe_for_quantum_chemistry.py)&emsp;
# [![View source on Gitee](https://mindspore-website.obs.cn-north-4.myhuaweicloud.com/website-images/r2.2/resource/_static/logo_source_en.svg)](https://gitee.com/mindspore/docs/blob/r2.2/docs/mindquantum/docs/source_en/case_library/vqe_for_quantum_chemistry.ipynb)&emsp;
...
# 
# The [generate_uccsd](https://www.mindspore.cn/mindquantum/docs/en/r0.9/algorithm/nisq/mindquantum.algorithm.nisq.generate_uccsd.html) function in the circuit module of MindSpore Quantum can be used to read the computing result saved in `molecule_file`, build the UCCSD wave function by one click, and obtain the corresponding quantum circuit.
...
# The molecule Hamiltonian uses [get_qubit_hamiltonian](https://www.mindspore.cn/mindquantum/docs/en/r0.9/algorithm/nisq/mindquantum.algorithm.nisq.get_qubit_hamiltonian.html) to read the previous computing result. The result is as follows:
...
# The unitary coupled-cluster operator $\hat{T} - \hat{T}^{\dagger}$ can be built using [uccsd_singlet_generator](https://www.mindspore.cn/mindquantum/docs/en/r0.9/algorithm/nisq/mindquantum.algorithm.nisq.uccsd_singlet_generator.html). Provide the total number of quantum bits (total number of spin orbits) and the total number of electrons, and set `anti_hermitian=True`.
...
# The `ucc_fermion_ops` built in the previous step is parameterized. Use the [Jordan-Wigner transformation](https://www.mindspore.cn/mindquantum/docs/en/r0.9/algorithm/nisq/mindquantum.algorithm.nisq.Transform.html#mindquantum.algorithm.nisq.Transform.jordan_wigner) to map the Fermi excitation operator to the Pauli operator:
...
# Next, we need to obtain the quantum circuit corresponding to the unitary operator $\exp{(\hat{T} - \hat{T}^{\dagger})}$. [TimeEvolution](https://www.mindspore.cn/mindquantum/docs/en/r0.9/core/operators/mindquantum.core.operators.TimeEvolution.html) can generate the circuit corresponding to $\exp{(-i\hat{H}t)}$, where $\hat{H}$ is a Hermitian operator, and $t$ is a real number. Note that when [TimeEvolution](https://www.mindspore.cn/mindquantum/docs/en/r0.9/core/operators/mindquantum.core.operators.TimeEvolution.html) is used, `ucc_qubit_ops` already contains the complex number factor $i$. Therefore, you need to divide `ucc_qubit_ops` by $i$ or extract the imaginary part of `ucc_qubit_ops`.
...
# ## Summary
# 
# In this case, the ground state energy of the LiH molecule is obtained by using scipy in two methods. In the first method, we use the [generate_uccsd](https://www.mindspore.cn/mindquantum/docs/en/r0.9/algorithm/nisq/mindquantum.algorithm.nisq.generate_uccsd.html) function packaged by MindSpore Quantum to generate a quantum neural network that can solve this problem. In the second method, we build a similar gradient operator step by step. The final results are the same.
