.. py:class:: mindquantum.core.operators.PolynomialTensor(n_body_tensors=None)
    ...
    :math:`n_\text{qubits}\times n_\text{qubits}` 矩阵和 :math:`n_\text{qubits}\times n_\text{qubits} n_\text{qubits}\times n_\text{qubits}` 矩阵中。
    ...
    **参数：**
    ...
    - **n_body_tensors** (dict) - 存储描述n-body作用的张量的字典。
      ...
      和 ``n_body_tensors[(1,1,0,0)]`` 将返回一个 :math:`n_\text{qubits}\times n_\text{qubits} n_\text{qubits}\times n_\text{qubits}` 的numpy数组。
      这些常数和数组分别表示indentity、 :math:`a^\dagger_p a_q` 和 :math:`a^\dagger_p a^\dagger_q a_r a_s` 的系数。默认值：None。
