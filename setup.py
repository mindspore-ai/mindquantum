...
class CMakeBuildExt(build_ext):  # pylint: disable=too-many-instance-attributes
    ...
    def cmake_install_fast_build(self):
        ...
        with fdopen(str(Path(self.build_dir) / 'CMakeCache.txt'), 'r') as cache_file:
            ...
            cmake_build_testing = [line.strip() for line in data if line.startswith('BUILD_TESTING')]
            if len(cmake_build_testing) == 1:
                build_testing = cmake_build_testing[0].split('=')[1]

        if not install_prefix or not build_testing:
            logging.info('Unable to locate the original installation prefix or BUILD_TESTING in %s', self.build_dir)
        ...
        for src_dir, _ in itertools.groupby(sorted(self.extensions, key=_src_dir_pred), key=_src_dir_pred):
            self.cmake_configure_build(
                str(src_dir),
                [
                    f'-DBUILD_TESTING:BOOL={build_testing}',
