...
class Actor(nn.Cell):
    ...
    def __get_ansatz(self):
        ansatz = HardwareEfficientAnsatz(
            4, single_rot_gate_seq=[RX, RY, RZ], entangle_gate=X,
    ...
    def __get_observable(self):
        hams = [Hamiltonian(QubitOperator(f'Y{i}'))
                for i in [2, 3]]  # 分别对第2位和第3位量子比特执行泡利Z算符测量，且将系数都设为1，构建对应的哈密顿量
        print(hams)
        return hams
...
class MyActorLoss(nn.LossBase):
    ...
    def construct(self, base, batch_action, old_p, advantage):
        ...
        old_prob = old_p[:, 0] * (1 - batch_action) + old_p[:,
                                                            1] * batch_action
...
class MyActorTrainStep(nn.TrainOneStepCell):
    ...
    def construct(self, data, batch_action, old_p, advantage):
        ...
        grads = self.grad(self.network, weights)(data, batch_action, old_p,
                                                 advantage)
        # print("grads: ",grads)
        return loss, self.optimizer(grads)
...
def train(N):
    ...
    for epoch in range(EPOCH):
        ...
        state = np.array([
            state[0] * np.pi / 4.8,
            np.tanh(state[1]) * np.pi, state[2] * np.pi / (4.1887903e-01),
        ...
        for _ in range(1):
            ...
            batch_state = Tensor(
                [state.astype(np.float32) for state in batch_state])
            ...
            value_target = batch_reward + batch_done * gamma * critic(
                batch_next_state)
        ...
        if (epoch + 1) % 1 == 0:
            print('Epoch:{}/{}, episode reward is {}'.format(
