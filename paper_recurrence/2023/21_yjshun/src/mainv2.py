...
class Main():
    def __init__(self,setname,Ent_name):
        ...
        self.Ent = Ent_name
        self.train_loader, self.test_loader = self.data_preporcess(TRAIN_SET_NUM, TEST_SET_NUM)
    ...
    def data_preporcess(self,trainset_num, testset_num):
        ...
        if self.setname=='FashionMNIST':
            self.dataset = h5py.File(self.dataset_file+'/FashionMNIST_1_2_wk.mat')
            ...
            self.dataset = scipy.io.loadmat(self.dataset_file+'/MNIST_1_9_wk.mat')
    ...
    def training(self):
        ...
        ansatz = Classifying_circuit(QUBIT_NUM,BLOCK_NUM,self.Ent).as_ansatz()
        circ = encoder + ansatz
        meas = [Hamiltonian(QubitOperator(f'Z{i}')) for i in [QUBIT_NUM-2, QUBIT_NUM-1]]  # 测量最后两个比特
