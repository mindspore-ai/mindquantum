...
def create_dataset(data, batch_size=5, repeat_size=1):
    ...
    """
    input_data = ds.GeneratorDataset(list(data),
...
if __name__ == "__main__":
    ...
    if use_additional_data:
        gammas, params = np.load(f"./data/{n_qubits}additional_data.npy",
                                 allow_pickle=True)
    else:
        gammas, params = np.load(f"./data/{n_qubits}qbsdata.npy",
                                 allow_pickle=True)
    # 删除 gamma=1的项
    ...
    print()
    print(
        f"train sample: {len(x_train)}, x_train shape: {x_train.shape}, y_train shape: {y_train.shape}"
    )
    print(
        f"test  sample: {len(x_test)}, x_test  shape: {x_test.shape }, y_test  shape: {y_test.shape}"
    )

    ds_train = create_dataset(zip(x_train, y_train),
                              batch_size=batch_size,
                              repeat_size=repeat_size)
    ds_test = create_dataset(zip(x_test, y_test),
                             batch_size=batch_size,
                             repeat_size=repeat_size)
    ...
    simulator = Simulator('mqvector', n_qubits=n_qubits)
    # 构建算期望和有关参数梯度的算子
    ...
    for epoch in range(epochs):
        ...
        print(
            f"epoch: {epoch+1:2}, training loss: {train_loss:8.6}, accuracy: {train_acc:5.4}, testing loss: {test_loss:8.6}, accuracy: {test_acc:5.4}"
        )
        ...
        if train_loss <= best_train_loss:
            ...
            save_checkpoint(
                QCNN,
                f"./result_{n_qubits}/{'interpolation' if use_additional_data else 'origin'}_with_best_train_loss.ckpt"
            )
        if train_acc >= best_train_acc:
            best_train_acc = train_acc
            save_checkpoint(
                QCNN,
                f"./result_{n_qubits}/{'interpolation' if use_additional_data else 'origin'}_with_best_train_acc.ckpt"
            )
        if test_loss <= best_test_loss:
            best_test_loss = test_loss
            save_checkpoint(
                QCNN,
                f"./result_{n_qubits}/{'interpolation' if use_additional_data else 'origin'}_with_best_test_loss.ckpt"
            )
        if test_acc >= best_test_acc:
            best_test_acc = test_acc
            save_checkpoint(
                QCNN,
                f"./result_{n_qubits}/{'interpolation' if use_additional_data else 'origin'}_with_best_test_acc.ckpt"
            )
    loss_acc = {}
    ...
    np.save(
        f"./result_{n_qubits}/{'interpolation' if use_additional_data else 'origin'}_loss_acc.npy",
        loss_acc)
