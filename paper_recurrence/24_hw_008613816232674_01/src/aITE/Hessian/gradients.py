...
def grad_circuit_symbolic_forward(circ):
    ...
    for i, gate in enumerate(circ):
        if isinstance(gate, (G.RX, G.RY, G.RZ)):
            ...
            circ_coeff_list.append(-1. / 2 *
                                   J)  #for example, grad(RX) = -j X RX
...
class Grad:
    ...
    def __init__(self, circ, pr, ham, n_qubits):
        ...
        self.circ_list, self.circ_coeff_list = grad_circuit_symbolic_forward(
            self.circ)

        assert len(self.circ_list) == len(self.k_list), '{} vs {}'.format(
            len(self.circ_list), len(self.k_list))
    ...
    def _grad(self, phase_shift):
        ...
        hess = np.zeros(
            (len(self.parameters), len(self.parameters))).astype(np.complex)

        for i, (circ_right,
                coeff) in enumerate(zip(self.circ_list, self.circ_coeff_list)):
            sim = Simulator('mqvector', self.n_qubits)
            circ_right.no_grad()
            grad_ops = sim.get_expectation_with_grad(self.ham, circ_right,
                                                     self.circ)
    ...
    def Hess_forwardMode(self):
        ...
        hess = np.zeros(
            (len(self.parameters), len(self.parameters))).astype(np.complex)
        for i, (circ_left, coeff_left) in enumerate(
                zip(self.circ_list, self.circ_coeff_list)):
            for j, (circ_right, coeff_right) in enumerate(
                    zip(self.circ_list, self.circ_coeff_list)):
                sim = Simulator('mqvector', self.n_qubits)
                circ_right.no_grad()
                circ_left.no_grad()
                ...
                grad_ops = sim.get_expectation_with_grad(
                    self.ham, circ_right, circ_left)
                e, g = grad_ops(self.parameters)
                hess[i][j] = e[0][
                    0] * coeff_left * coeff_right * phase_shift * phase_shift  #* J
