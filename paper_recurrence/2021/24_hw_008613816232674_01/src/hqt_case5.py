...
class Hamiltonian_1D:
    ...
    def local_Hamiltonian_with_pauli(self, f_first, f_last, pauli_type='X'):
        if pauli_type == 'I':
            ...
            ham = self._local_Hamiltonian_with_pauli(f_first, f_last,
                                                     pauli_type)
        ham = Hamiltonian(ham)
        return ham
    ...
    def _local_Hamiltonian_with_pauli(self, f_first, f_last, pauli_type='X'):
        ...
        for i in range(self.n_qubits):
            if ham is None:
                ham = QubitOperator(
                    'X{} {}{}'.format(i, pauli_type, self.n_qubits), g)
            else:
                ham += QubitOperator(
                    'X{} {}{}'.format(i, pauli_type, self.n_qubits), g)

            ham += QubitOperator(
                'Z{} {}{}'.format(i, pauli_type, self.n_qubits), h)
            if i < self.n_qubits - 1:
                ham += QubitOperator(
                    'Z{} Z{} {}{}'.format(i, i + 1, pauli_type, self.n_qubits),
                    1)

        i, j = 0, self.n_qubits - 1
        ham += QubitOperator('Z{} {}{}'.format(i, pauli_type, self.n_qubits),
                             f_first)
        ham += QubitOperator('Z{} {}{}'.format(j, pauli_type, self.n_qubits),
                             f_last)
        ...
        for k in self._dict.keys():
            if k[qubit_index] == key:
                ...
                if self.verbose_dict[qubit_index][key] != 0:
                    _dict[k] = self._dict[k] / self.verbose_dict[qubit_index][
                        key]
                else:
                    _dict[k] = 0
...
class VQE_1D:
    ...
    def __init__(self, n_subsystems=3, n_qubits_per_subsystems=8):
        ...
        for i in range(len(interaction_coeff) - 1):
            self.boundary_pairs.append(
                [interaction_coeff[i], interaction_coeff[i + 1]])
        ...
        self.layer1_circ = self.qt.layer(P,
                                         n_qubits=n_subsystems,
                                         layer_type='first')
        ...
        for i in range(n_subsystems):
            ...
            circ = self.qt.layer(P,
                                 n_qubits=n_qubits_per_subsystems + 1,
                                 layer_type='second')
    ...
    def norm(self):
        ...
        ham = Hamiltonian(qo)
        sim = Simulator('mqvector', self.n_subsystems)
    ...
    def layer1_operator(self):
        ...
        for i in range(n_subsystems):
            ...
            nv.initialize_single(
                *[expectation[x] for x in ['I', 'X', 'Y', 'Z']])
            if V is None:
                V = nv
            else:
...
class OPT_VQE1D(VQE_1D):
    ...
    def __init__(self,
                 ...
                 lr=0.1,
                 opt_type='aite'):
        super().__init__(n_subsystems=n_subsystems,
                         n_qubits_per_subsystems=n_qubits_per_subsystems)
        self.layer1_optimizer = Optimizer(self.layer1_circ,
                                          self.layer1_pr,
                                          n_subsystems,
                                          opt_type=opt_type,
                                          lr=lr)
        ...
        for i in range(self.n_subsystems):
            ...
            self.layer2_optimizer[i] = Optimizer(circ,
                                                 pr,
                                                 n_qubits,
                                                 opt_type=opt_type,
                                                 lr=lr)
    ...
    def update_layer1_gradient(self, qubit_index, pauli_key):
        ...
        sim = Simulator('mqvector', n_subsystems)
        sim.apply_circuit(self.layer1_circ, pr=self.layer1_pr)
        E = sim.get_expectation(ham)

        self.layer1_gradient[(
            qubit_index,
            pauli_key)] = E  #* QubitOperator_helper().get_coeff()[pauli_key]
...
if __name__ == '__main__':
    ...
    m = OPT_VQE1D(n_subsystems=n_subsystems,
                  n_qubits_per_subsystems=n_qubits_per_subsystems,
                  lr=0.05,
                  opt_type='aite')

    from benchmark.mps import MPS
    M = MPS(n_subsystems=n_subsystems,
            n_qubits_per_subsystems=n_qubits_per_subsystems,
            bond_dim=32,
            interaction_coeff=m.interaction_coeff)
    M.run()
    ...
    for i in range(10000):
        ...
        if i % 2 == 0:
            ...
            print(E.real, norm.real, (E / norm).real,
                  (E / norm).real / (n_subsystems * n_qubits_per_subsystems))
