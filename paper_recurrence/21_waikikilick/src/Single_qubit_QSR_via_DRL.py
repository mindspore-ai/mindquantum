...
context.set_context(mode=context.GRAPH_MODE,
                    device_target="CPU")  # 设置静态图模式，CPU 支持此模式
...
class LinearNet(nn.Cell):
    ...
    def __init__(self):
        super(LinearNet,
              self).__init__()  # 网络： 两个隐藏层，分别有 32/32 个神经元；输出层 4 个神经元。
...
class Agent(nn.Cell):
    ...
    def __init__(self,
        ...
        self.main_net = Model(net, nn.loss.MSELoss(),
                              nn.RMSProp(params=net.trainable_params(),
                                         learning_rate=self.lr))  # 主网络
    ...
    def get_train_data(self, s_batch, Q):
        train_data = ds.GeneratorDataset(list(self.get_samples(s_batch, Q)),
                                         column_names=['state', 'Q'])
    ...
    def learn(self):
        ...
        batch_memory = random.sample(self.memory,
                                     self.batch_size)  # 从 记忆库 中随机选出 批大小 数量的样本
        s_batch = np.array([replay[0] for replay in batch_memory
                            ]).astype(np.float32)  # 多行数组
        Q = self.main_net.predict(Tensor(s_batch))  # 主网络预测 Q 值

        next_s_batch = np.array([replay[3] for replay in batch_memory
                                 ]).astype(np.float32)
        Q_next = self.target_net.predict(
            Tensor(next_s_batch))  # 目标网络预测下一时刻状态的 Q 值
...
class env():
    ...
    def psi_set(self):
        ...
        psi_set.append(np.array([1, 0]).astype(
            np.complex64))  # 最后再加上 |0> 和 |1> 两个态
    ...
    def reset(self, sim, init_psi):  # 在一个新的回合开始时，归位到开始选中的那个点上
        sim.set_qs(init_psi)
        init_state = np.array([
            init_psi[0].real, init_psi[1].real, init_psi[0].imag,
            init_psi[1].imag
        ])
    ...
    def step(self, sim, sim_target, action, nstep):
        ...
        psi_ = sim.get_qs()
        state_ = np.array(
            [psi_[0].real, psi_[1].real, psi_[0].imag, psi_[1].imag])

        err = 10e-4
        rwd = fid
        done = (((1 - fid) < err)
                or nstep >= 2 * np.pi / dt)  # 用于判断智能体是否停止当前任务
...
def training(ep_max):
    ...
    for i in range(ep_max):
        ...
        while True:
            ...
            sim, observation_, reward, done, fid = env.step(
                sim, sim_target, action, nstep)
            nstep += 1
        ...
        if i % 1 == 0:  # 每 x 个回合用验证集验证一下效果，动作全靠网络，保存最大保真度和最大奖励
            ...
            for validation_init_psi in validation_set:
                ...
                while True:
                    ...
                    validation_reward_tot = validation_reward_tot + reward * (
                        agent.gamma**nstep)
                    observation = observation_
            ...
            validation_reward_history.append(
                np.mean(validation_reward_tot_list))
...
if __name__ == "__main__":
    ...
    fig, ax = plt.subplots()
    ax.plot(x,
            np.array(validation_reward_history) / 7,
            '--',
            label='validation reward history')
    ax.plot(x,
            validation_fid_history,
            ':',
            label='validation fidelity history')
