...
class Hackathon03:
    ...
    def init_model(self):
        ...
        self.sim_m = Simulator('projectq', self.n_qubits)
        grad_ops = sim.get_expectation_with_grad(self.ham,
                                                 self.circuit,
                                                 Circuit(),
                                                 self.sim_m,
                                                 self.epn,
    ...
    def train(self, train_x, train_y, test_x, test_y, batch=10, weights=None,
              acc_tol=1, method='bfgs', iter_info=False, tol=1e-5):
        """
        Train.
        """
        acc = -1
        ...
        for i in range(int(train_x.shape[0]/batch)):
            ...
            acc_ = predict_acc(test_x, test_y,
                               self.circuit, weights_ ,
                               self.epn, self.apn)
...
if __name__ == '__main__':
    ...
    model = Hackathon03()
    model.train(train_x[:100], train_y[:100], test_x, test_y,
                acc_tol=0.4, iter_info=True, tol=1e-2)
    print(model.weights)
