...
def generate_initial_quantum_state(train_data):
    ...
    encoder, paras_name = generate_encoder()  # generate encoder circuit and circuit's parameter's name
...
def _cost_function(parameters):
    ...
    phi = [predict_quantum_state(psi, parameters) for psi in initial_quantum_data]
    cost = [(1-abs(np.vdot(train_y[i], phi[i]))**2)**2 for i in range(len(phi))]
    cost = 1/len(phi)*sum(cost)
    
    phi_test = [predict_quantum_state(psi, parameters) for psi in train_test_quantum_data]
    acc = np.real(np.mean([np.abs(np.vdot(normal(bra), ket)) for bra, ket in zip(phi_test, train_y[750:])]))
...
def pred_accuracy(parameters, quantum_state, real_quantum_state): # this quantum state is 18
    
    phi = [predict_quantum_state(psi, parameters) for psi in quantum_state]
    acc = np.real(np.mean([np.abs(np.vdot(normal(bra), ket)) for bra, ket in zip(phi, real_quantum_state)]))
