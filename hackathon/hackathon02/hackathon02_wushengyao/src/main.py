...
#         self.circuit = None
    ...
    def select_valid_excitations(self):
        singles, doubles = get_possible_excitations(self.n_electrons, self.n_qubits, delta_sz=0)
        ...
        gd = gd[0][0].real
        self.double_excitations = [doubles[i] for i in range(len(doubles)) if abs(gd[i]) > 1.0e-5]
        ...
        select_singles_circuit1, select_singles_circuit2 = self.circuit_2(singles, self.double_excitations)
        select_singles_circuit1 = select_singles_circuit1.no_grad()
        ops_singles_circuit = select_singles_circuit1 + select_singles_circuit2

        grad_ops = self.simulator.get_expectation_with_grad(
            self.hamiltonian,
            ops_singles_circuit,
            encoder_params_name=select_singles_circuit1.params_name,
            ansatz_params_name=select_singles_circuit2.params_name)

        params_s = [0] * len(singles)
        e, ge, gs = grad_ops(np.array([params_d]), np.array(params_s))
        gs = gs[0][0].real
        self.single_excitations = [singles[i] for i in range(len(singles)) if abs(gs[i]) > 1.0e-5]
    ...
    def circuit_2(self, excitations, excitation_select):
        circ1 = Circuit()
        circ1 += Circuit([X.on(i) for i in range(self.n_electrons)])  # hf state
    ...
    def optimize(self, operator=None, init_amp=[], method='bfgs', maxstep=200, iter_info=False):
        if operator == None:
        ...
        grad_ops = self.simulator.get_expectation_with_grad(operator, self.circuit)
