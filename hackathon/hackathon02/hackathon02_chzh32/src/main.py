...
class VQEoptimizer:
    def __init__(self,
                 molecule=None,
                 file=None):  # molecule：分子数据 file：中间数据打印地址
        self.timer = Timer()  # 开始计时
        self.molecule = molecule
        self.backend = 'mqvector'  # 后端模拟器
        self.file = file

        if molecule != None:
            self.generate_circuit(molecule)  # 根据分子数据，生成所有量子线路

        print('Initialize finished! Time: ',
              self.timer.runtime(),
              file=self.file)
    ...
    def generate_circuit(self, molecule=None, ansatz_layers=11):
        ...
        self.circuit = Circuit([H.on(i)
                                for i in range(self.n_qubits)])  # 生成编码线路
        ...
        self.circuit += Ansatz(num_qubit=self.n_qubits,
                               num_layer=ansatz_layers)  # 整体量子线路 = 编码线路 + 拟设线路
        self.simulator = Simulator(self.backend, self.n_qubits)
        grad_ops = self.simulator.get_expectation_with_grad(
            Hamiltonian(self.hamiltonian), self.circuit)
        self.qnet = MQAnsatzOnlyLayer(grad_ops)

    def optimize(self):
        optimizer = ms.nn.Adam(self.qnet.trainable_params(),
                               learning_rate=0.01)
...
def Ansatz(num_qubit=8, num_layer=4):
    ...
    for ii in range(num_layer):
        for jj in range(0, num_qubit, 2):
            ansatz += conv_circ(prefix='%s' % (ii) + '_0_' + '%s' %
                                (int(jj / 2)),
                                bit_up=jj,
                                bit_down=jj + 1)
        ...
        for jj in range(1, num_qubit - 1, 2):
            ansatz += conv_circ(prefix='%s' % (ii) + '_1_' + '%s' % (int(
                (jj - 1) / 2)),
                                bit_up=jj,
                                bit_down=jj + 1)
        ansatz += conv_circ(prefix='%s' % (ii) + '_1_' + '%s' % (int(
    ...
    def run(self, prefix, molecular_file,
        ...
        if prefix == 'LiH':
            ...
            with open(self.work_dir + prefix + '.o', 'a') as f:
                ...
                vqe = VQEoptimizer(
                    file=f)  # file 参数和 print 的 file 参数，为了将中间数据打印并保存到 .o 文件。
                en_list, time_list = [], []
                ...
                mol = run_pyscf(mol0, run_scf=0, run_ccsd=1,
                                run_fci=1)  # 运行得到 FCI 法，得到参考能量值
                ...
                print('Our Energy: ',
                      en,
                      'Time: %i hrs %i mints %.2f sec.' % format_time(t),
                      file=f)
                print('Precision compared with FCI is:',
                      np.abs(en - mol.fci_energy),
                      file=f)
                en_list.append(en[0])
                time_list.append(t)

                print(
                    'Optimization completed. Time: %i hrs %i mints %.2f sec.' %
            ...
            if len(en_list) == len(geom_list) and len(time_list) == len(
                    geom_list):
            ...
            return list([0. for _ in range(len(geom_list))
                         ]), list([1 for _ in range(len(geom_list))])
