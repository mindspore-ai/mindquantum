...
class VQEoptimizer:
    ...
    def generate_circuit(self, molecule=None):
        ...
        ansatz_1 = Circuit(UN(H, range(self.n_qubits))) + Circuit(
            UN(H, range(self.n_qubits)))
    ...
    def optimize(self,
        ...
        self.res = minimize(
            ...
            method=
            method,  # 求解的算法，目前可选的有 ‘Nelder-Mead’‘Powell’‘CG’‘BFGS’‘Newton-CG’‘L-BFGS-B’‘TNC’‘COBYLA’‘SLSQP’‘dogleg’‘trust-ncg’ 
            # 以及在 version 0.14.0，还能自定义算法 以上算法的解释和相关用法见 minimize 函数的官方说明文档，一般求极值多用 'SLSQP'算法
            jac=
            True,  # 目标函数的雅可比矩阵。可选项，仅适用于CG，BFGS，Newton-CG，L-BFGS-B，TNC，SLSQP，dogleg，trust-ncg。
    ...
    def run(self, prefix, molecular_file,
        ...
        with open(self.work_dir + prefix + '.o', 'a') as f:
            ...
            for i in range(len(geom_list)):
                ...
                vqe.simulator.apply_circuit(vqe.circuit,
                                            param_dict)  # 执行设计好的 网络 及 参数
                t = vqe.timer.runtime()
