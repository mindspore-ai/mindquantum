...
from MPS import train_mps_basic_state, parse_hamiltonian, mps2circparams
...
def get_best_mps_params(ham, mps_circ, p0=None):
    circ = mps_circ
...
def get_optimal_scaling_circit_params(circuit, split_ham, Simulator, shots=100, scaling=[1.0, 2.0, 3.0],
                                      Z_hams_is=True, ham=None, p0=None):
    ...
    for repetition in range(n_circuit):
        params = get_best_mps_params(ham, circuit, p0=p0)
...
def solution(molecule, Simulator: simulator.HKSSimulator) -> float:
    ...
    print("ucc info:", ucc)
    loss_list, energy_list, trained_mps_tensors = train_mps_basic_state(iter=2000, H=ham.__str__(), target_energy=mol.fci_energy)
    tc.save(trained_mps_tensors, 'trained_mps_tensors.pt')
    pretrained_circ_params = mps2circparams(trained_mps_tensors, iter=300, filename='trained_mps_tensors.pt')


    p, min_expec_for_scaling1 = get_optimal_scaling_circit_params(ucc, split_ham, Simulator, shots=100, scaling=scaling, Z_hams_is=group_Z_hams_is, ham=ham, p0=pretrained_circ_params)
