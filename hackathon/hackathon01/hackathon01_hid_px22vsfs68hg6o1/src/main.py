...
class Main(HybridModel):
    ...
    def build_grad_ops(self):
        ...
        for j in range(7):  # j = 0,1,2,3,4,5,6
            ...
            encoder += RZ(f'alpha{j + 8}').on(j + 1)  # RZ(alpha_{j+8})门作用在第0位量子比特
            encoder += X.on(j + 1, j)  # X门作用在第j+1位量子比特，受第j位量子比特控制
        ...
        print(encoder)
        #ansatz:
        ansatz = HardwareEfficientAnsatz(8, single_rot_gate_seq=[RY], entangle_gate=X,
        ...
        #hamiltonian
        #对什么位进行量子比特测量区别也很大
        hams = [Hamiltonian(QubitOperator(f'Z{i}')) for i in [3, 4]]  # 分别对第3位和第4位量子比特执行泡利Z算符测量，且将系数都设为1，构建对应的哈密顿量
        print(hams)
    ...
    def train(self):
        ...
        self.model.train(8, self.dataset, callbacks=[monitor,acc],dataset_sink_mode=False) # 将上述建立好的模型训练8次
        print("finished!")
