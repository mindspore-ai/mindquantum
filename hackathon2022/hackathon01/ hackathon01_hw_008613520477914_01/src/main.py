...
def gen_enhanced_samples(img_use_feas, lab_use):
    ...
    for i in range(len(img_use_feas)):
        ...
        for pix_id in range(len(item)):
            if item[pix_id] == 0:
                img_use_feas_copy[i,pix_id] = 0.1*(np.random.uniform()-0.95)
            if item[pix_id] == 1:
                img_use_feas_copy[i,pix_id] = (np.random.uniform()*0.2 + 0.95)
...
class Main(HybridModel):
    ...
    def build_grad_ops(self):
        ...
        ansatz =  HardwareEfficientAnsatz(8,[RX,RZ,RY],X,'linear',4).circuit
    ...
    def build_model(self):
        ...
        self.opti = ms.nn.Adam(self.qnet.trainable_params(), learning_rate = 0.015)
