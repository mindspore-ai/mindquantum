...
class Main(HybridModel):
    def __init__(self):
        super().__init__()
        self.dataset = self.build_dataset(self.origin_x, self.origin_y, 25)#加载xunlian数据
    ...
    def build_grad_ops(self):
        ...
        ansatz = HardwareEfficientAnsatz(8, single_rot_gate_seq=[RY], entangle_gate=X, depth=12).circuit  # 通过HardwareEfficientAnsatz搭建Ansatz
    ...
    def build_model(self):
        #损失函数
        self.loss = ms.nn.SoftmaxCrossEntropyWithLogits(sparse=True)# 通过SoftmaxCrossEntropyWithLogits定义损失函数，sparse=True表示指定标签使用稀疏格式，reduction='mean'表示损失函数的降维方法为求平均值
        
        #adam下降
        self.opti = ms.nn.Adam(self.qnet.trainable_params())# 通过Adam优化器优化Ansatz中的参数，需要优化的是Quantumnet中可训练的参数，学习率设为0.1
    ...
    def train(self):
        ...
        self.model.train(10, self.dataset, callbacks=LossMonitor(200))# 监控训练中的损失，每64步打印一次损失值,10个人epoch
       # 对训练数据集进行预测
