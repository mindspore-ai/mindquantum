...
class Net(ms.nn.Cell):
    def __init__(self):
        ...
        self.MQLayer1 = MQLayer(build_grad_ops(build_encoder(first=True), 
                                               build_ansatz(), 
                                               num_output=4))
        self.MQLayer2 = MQLayer(build_grad_ops(build_encoder(first=False), 
                                               build_ansatz(), 
                                               num_output=4))
        self.MQLayer3 = MQLayer(build_grad_ops(build_encoder(first=False),  
                                               build_ansatz(), 
                                               num_output=1))
...
class Main(HybridModel):
    def __init__(self):
        ...
        self.train_loader, self.test_loader = self.build_dataset(self.origin_x, self.origin_y, 10) 
    ...
    def build_dataset(self, x, y, batch=None):
        ...
        train_loader = ds.NumpySlicesDataset({"image": train_x, "label": train_y.astype(np.float32)}, shuffle=True)
        test_loader = ds.NumpySlicesDataset({"image": test_x, "label": test_y.astype(np.float32)})
    ...
    def build_model(self):
        ...
        self.opti = ms.nn.Adam(self.qnet.trainable_params(), learning_rate=0.001)
        self.model = Model(self.qnet, self.loss, self.opti)
        return self.model


    def train(self):
        self.model.train(20, self.train_loader, callbacks=LossMonitor(100), dataset_sink_mode=False)
