...
class Main():
    def __init__(self, dep=3, learning_rate=0.5, test_size=0.1):
        ...
        X_train, X_test, Y_train, Y_test = train_test_split(self.X, self.Y, test_size=test_size, random_state=0, shuffle=False)
        X_test[:,18:] = 0
    ...
    def trans_data(self, X, Y):
        ...
        for i, item in enumerate(Y):
            ...
            th_pa = [ths[7]-ths[3], ths[5]-ths[1],ths[6]-ths[2], ths[4]-ths[0], (ths[2] + ths[6] -ths[0]-ths[4])/2, (ths[3] + ths[7]-ths[1]-ths[5])/2, ((ths[3] + ths[7]+ ths[1]+ths[5])/2 - (ths[0]+ths[2] + ths[4] + ths[6])/2)/2]
            trans_x.append(list(X[i]) + th_pa[::-1] + am_pa[::-1])
            trans_y.append([3])
        return np.array(trans_x).astype(np.float32), np.array(trans_y).astype(np.float32)

    def set_phase_para(self, circ, ths):
        theta = [ths[7]-ths[3], ths[5]-ths[1],ths[6]-ths[2], ths[4]-ths[0], (ths[2] + ths[6] -ths[0]-ths[4])/2, (ths[3] + ths[7]-ths[1]-ths[5])/2, ((ths[3] + ths[7]+ ths[1]+ths[5])/2 - (ths[0]+ths[2] + ths[4] + ths[6])/2)/2]
    ...
    def build_grad_ops(self):
        ...
        total_circ = encoder + ansatz + end_coder
        print(total_circ.parameter_resolver())
        print(encoder_params_name)
        self.circ = total_circ
      
        ham = [Hamiltonian(QubitOperator(f'Z0',-1) + QubitOperator(f'Z1',-1) + QubitOperator(f'Z2',-1))] 
    ...
    def bulid_net(self):
        opti = ms.nn.Adam(self.qnet.trainable_params(), learning_rate=self.learning_rate)   
    ...
    def train_net(self, steps):
        for i in range(steps):
            ...
            if i % 10 == 0:
                ...
                self.verify_circ(ansatz_values, self._raw_x[-50:], self._raw_y[-50:])
