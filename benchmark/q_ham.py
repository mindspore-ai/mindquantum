...
class MolInfoProduce(object):
    def __init__(self, geometry, basis, charge, multiplicity, fermion_transform):
    ...
    def mol_info_producer(self):
        mol = MolecularData(geometry=self.geometry, basis=self.basis, 
                            charge=self.charge, multiplicity=self.multiplicity)

        py_mol = run_pyscf(mol, 
                      run_scf=1,
                      run_ccsd=1,
                      run_fci=1)
        ...
        inter_ops = InteractionOperator(*molecular_hamiltonian.n_body_tensors.values())
...
def q_ham_producer(geometry, basis, charge, multiplicity, fermion_transform):
    mol = MolecularData(geometry=geometry, basis=basis, charge=charge, multiplicity=multiplicity)
    py_mol = run_pyscf(mol, 
                      run_scf=1,
                      run_ccsd=1,
                      run_fci=1)
    ...
    return (py_mol.n_qubits, py_mol.n_electrons, 
            py_mol.hf_energy, py_mol.ccsd_energy,
            py_mol.fci_energy, q_ham.real) 
