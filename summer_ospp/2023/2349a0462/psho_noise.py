...
# PSHO 间接法
# 使用 openfermion 生成分子数据，可以在本地 Linux 环境下运行
# 在构造量子线路和求期望值时，都去掉了哈密顿量中的常数项
# 以 H2 分子为例
# 时间演化因子采用二阶 Trotter 近似
# 将辅助比特放在了最后一位
...
ham_HZ = Hamiltonian((ham_mol - QubitOperator(constant_term, constant)) * QubitOperator(f'Z{mol_n_qubits}', -1)) # 论文中 -ZH 的期望值 （去掉了常数项）
# 形如： -0.1714 [Z0 Z4] + -0.1714 [Z1 Z4] + ...

ham_IZ = Hamiltonian(QubitOperator(f'Z{mol_n_qubits}', -1))                # 论文中 -ZI 的期望值
# 形如： -1 [Z4] 
...
tau = 1.4 # 演化时间 tau = (np.pi/2) / E_0
delta = 0.1
for amp in [0.00001, 0.0001, 0.001, 0.01, 0.1]: # 噪声幅值
    n = 10
    ### 求解分子和分母的第一项：
    An1 = 0
    Bn1 = 0
    ...
    for k in range(n):
        ...
        circ += S.on(circ1.n_qubits)
        circ += H.on(circ1.n_qubits)
        circ = circ.with_noise(AmplitudeDampingChannel(amp), also_ctrl=True)
    ...
    sim2.apply_circuit(hatree.with_noise(AmplitudeDampingChannel(amp), also_ctrl=True))
