...
def create_ansatz_circuit(n,p):
    ansatz_p = HardwareEfficientAnsatz(n, single_rot_gate_seq=[RY], entangle_gate=X, depth=p).circuit     # 通过HardwareEfficientAnsatz搭建Ansatz
    ansatz_p.as_ansatz()
    return  ansatz_p
...
# 获取当前量子态信息,存储在字典info中，info = {'十进制k':phi_k},k = 0,1,2...N-1
...
def get_expectation(n,layer,ansatz_data):
    def execute_circ(ansatz_data):
        return cost_function(n,layer,ansatz_data)
    return execute_circ
...
def train_parameters(layer,counts,ansatz_data):
   # 记录深度为layer时，优化后函数值和优化后参数之间的对应关系
    ...
    for times in range(1,counts+1):
        ...
        # 参数优化部分
        out = minimize(fun = get_expectation(n,layer,initial_ansatz), x0 = initial_ansatz,tol=10e-8, method="COBYLA", options={'disp':True,'maxiter':500})
#         out = minimize(fun = get_expectation(n,layer,encoder_data,initial_ansatz), x0 = initial_ansatz,tol=10e-8, method="Nelder-Mead", options={'disp':True,'maxiter':1000})
        print(out)
...
def global_training(n,p):
    ...
    # 最小化代价函数
    # out = minimize(fun = get_expectation(n,p,initial_ansatz), x0 = initial_ansatz, method="COBYLA", options={'disp':True,'maxiter':1000})

    out = minimize(fun = get_expectation(n,p,initial_ansatz), x0 = initial_ansatz, method="BFGS",options={'eps':1e-06,'disp':True,'maxiter':400}) 
    print(out)
...
# 向字典pr中添加参数信息，获取encoder以及ansatz_data的完整信息
pr = {}
