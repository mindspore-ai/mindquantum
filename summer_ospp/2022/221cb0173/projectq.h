...
inline ThreadPool::ThreadPool(size_t threads) //initize threadpool
    :   stop(false)
{
...
template<class F, class... Args>
auto ThreadPool::enqueue(F&& f, Args&&... args)  //enqueue task
...
inline ThreadPool::~ThreadPool() //stop threadpool
{
    {
        std::unique_lock<std::mutex> lock(queue_mutex);
        stop = true;
...
ThreadPool* ThreadPool::p=new ThreadPool(8); //create threadpool
 ...
 public:
    ...
    VT<VT<CT<T>>> HermitianMeasureWithGrad(const VT<Hamiltonian<T>> &hams, const VT<BasicGate<T>> &circ,
        ...
        if (n_hams == 1) {
            ...
            ThreadPool* pp=ThreadPool::getInstance(); //get threadpool
            for (size_t i = 0; i < mea_threads; ++i) {
                ...
                pp->enqueue(task); // push tasks into threadpool
            }
            while(1) //wait for all tasks
            {
                if(pp->idle==0&&pp->tasks.empty())
                {
                    break;
                }
            }
