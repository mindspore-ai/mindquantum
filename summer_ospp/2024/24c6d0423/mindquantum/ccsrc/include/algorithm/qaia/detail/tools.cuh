...
struct Bounds<int8_t> {
    static __device__ int8_t UP() { return 127; }
    static __device__ int8_t LOW() { return -127; }
};

// 对 half 进行特化
template <>
struct Bounds<half> {
    static __device__ half UP() { return __float2half(1.0f); }
    static __device__ half LOW() { return __float2half(-1.0f); }
};
...
__global__ void update_tail_half(
    const half *__restrict__ tmp, half *x, half *y, int size, float xi, float beta, float delta, float dt
){
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= size) return;
...
__global__ void update_h_tail_half(
    const half *__restrict__ tmp, half *x, half *y, half* h, int size, float xi, float beta, float delta, float dt
){
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= size) return;
...
__global__ void update_tail(
    const int *__restrict__ tmp, int8_t *x, int *y, int size, float xi, float beta, float delta, float dt
){
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= size) return;
...
__global__ void update_h_tail(
    const int *__restrict__ tmp, int8_t *x, int *y, int* h, int size, float xi, float beta, float delta, float dt
){
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= size) return;
...
template <>
struct Value<int8_t> {
    static int8_t v(double value) { return static_cast<int8_t>(value * 127.0); }
};

template <>
struct Value<half> {
    static half v(double value) { return static_cast<half>(value); }
};
