...
import csv
def save_lists_to_csv_by_column(file_path, data_lists):
    try:
        transposed_data = zip(*data_lists)
...
'''
    绘图函数
'''
def draw_pr(gama, beta, path):
    layer_f1 = np.linspace(1,10,10)

    plt.figure(figsize=(8, 6), dpi=120, facecolor='lightgray')
    plt.plot(layer_f1, gama, color='green', linestyle='--', marker='d', markersize=7, label='Cost layer')
    plt.plot(layer_f1, beta, color='green', linestyle='-', marker='d', markersize=7, label='Mixer layer')
    plt.plot(layer_f1,np.full_like(layer_f1,-np.pi/4),color='grey',linestyle=':',linewidth=3,label='-pi/4')
    plt.plot(layer_f1,np.full_like(layer_f1, np.pi/4),color='grey',linestyle=':',linewidth=3,label='pi/4')
    plt.xlabel('Layer index l<=p', fontsize=16)
    plt.ylabel('Parameters', fontsize=16)
    plt.title('ADAPT-QAOA solution p = 10', fontsize=16)
