...
class Clifford(StabilizerTableau):
    def __init__(self, table=None, phase=None, num_qubits=None):
        super(Clifford, self).__init__(table=table, phase=phase, num_qubits=num_qubits)
            ...
            for i in range(n):
                ...
                if np.any(cli_copy._table[i,(i+n):]):
                    if not cli_copy._table[i,i+n]:
                        cli_copy.Pdagger(i)  # Pd|psi>=|0> thus |psi>=P|0>
                ...
                if np.any(cli_copy._table[i+n,i:n]):
                    cli_copy.Hadamard(i)
    ...
    def apply_circuit(self, circuit_str, reverse=False):
        if circuit_str is None: return
        for igate in circuit_str:
            if igate[0]=='C':
                ...
                if reverse: self.Pdagger(igate[1])
                else: self.PhaseGate(igate[1])
            elif igate[0]=='X':
                self.XGate(igate[1])
            elif igate[0]=='Z':
                self.ZGate(igate[1])
