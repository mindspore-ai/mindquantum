...
def _para_uccsd_singlet_generator(mol, th=0):
    ...
    Args:
        ...
        th (float, optional): A threadhold of parameters. If the absolute value of parameter is
...
def generate_uccsd(molecular, th=0):
    ...
    Args:
        ...
        th (float): the threshold to filt the uccsd amplitude. We only keep the
            excitation operator with absolute value of amplitude greater than `th`, so
            that if `th=0`, we only keep excitation operator with non zero amplitude. Default: 0.
