...
def random_rng(seed: int = None):
    """
    Generate a random generator according to current time.

    Args:
        seed (int): The random seed. If ``None``, the seed will be generated by current time. Default: ``None``.
    """
    if seed is None:
        seed = int(time.time() * 1000 % 1 * 10000 // 1)
