...
def _check_and_generate_pr_type(pr, names: list):
    ...
    if isinstance(pr, _num_type):
        if len(names) != 1:
            raise ValueError(
                f"number of given parameters value is less than parameters ({len(names)})"
            )
        pr = np.array([pr])
    if not isinstance(pr, ParameterResolver) and not isinstance(
            pr, np.ndarray):
        raise TypeError(
            f"parameter requires a parameter resolver or a numpy array, but get type{type(pr)}"
        )

    if isinstance(pr, np.ndarray):
        if len(pr) != len(names) or len(pr.shape) != 1:
            raise ValueError(
                f"given parameter value size ({pr.shape}) not match with parameter size ({len(names)})"
            )
