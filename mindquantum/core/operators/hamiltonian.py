    ...
    def get_cpp_obj(self, hermitian=False):
        ...
        if not hermitian:
            if self.ham_cpp is None:
                if self.how_to == HowTo.ORIGIN:
                    ham = getattr(mb, get_context('dtype')).hamiltonian(self.ham_termlist)
                elif self.how_to == HowTo.BACKEND:
                    ham = getattr(mb, get_context('dtype')).hamiltonian(self.ham_termlist, self.n_qubits)
                else:
                    dim = self.sparse_mat.shape[0]
                    nnz = self.sparse_mat.nnz
                    csr_mat = getattr(mb, get_context('dtype')).csr_hd_matrix(
                        dim, nnz, self.sparse_mat.indptr, self.sparse_mat.indices, self.sparse_mat.data
                    )
                    ham = getattr(mb, get_context('dtype')).hamiltonian(csr_mat, self.n_qubits)
                self.ham_cpp = ham
            return self.ham_cpp
        ...
        if self.herm_ham_cpp is None:
            ...
            self.herm_ham_cpp = getattr(mb, get_context('dtype')).hamiltonian(csr_mat, self.n_qubits)
