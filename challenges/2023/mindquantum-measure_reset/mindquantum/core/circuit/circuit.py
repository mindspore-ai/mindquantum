...
class CollectionMap:
    ...
    def __rename(self,current_name,new_name):
        key_list = list(self.map.keys())
        for g in key_list:
            if g.key == current_name:
                x=self.map.pop(g)
                g.key = new_name
                self.map[g]=1
        return None

    def collect_only_onemeasure(self,keys,raise_msg):
        """Collect item only single time, otherwise raise error. But this is special version for measurement gate with reset function"""
        if not isinstance(keys, list):
            keys = [keys]
        for k in keys:
            if k in self.map:
                last_reset = self.__get_reset(k.key)              
                if last_reset == True:
                    new_last_name = "mid_"+k.key+"_"+str(self.map[k])
                    Num = self.map[k]
                    self.__rename(k.key,new_last_name)
                    self.map[k] = Num+1
                else:
                    raise ValueError(raise_msg)
            else:
...
class Circuit(list):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def append(self, gate):
        ...
        if isinstance(gate, mq_gates.Measure):
            self.all_measures.collect_only_onemeasure(gate, f'measure key {gate.key} already exist.')
    ...
    def __setitem__(self, k, v):
        ...
        if isinstance(v, mq_gates.Measure):
            self.all_measures.collect_only_onemeasure(v, f'measure key {v.key} already exist.')
    ...
    def insert(self, index, gates):
        ...
        if isinstance(gates, mq_gates.BasicGate):
            ...
            if isinstance(gates, mq_gates.Measure):
                self.all_measures.collect_only_onemeasure(gates, f'measure key {gates.key} already exist.')
    ...
    def xx(self, para, obj_qubits, ctrl_qubits=None):
        ...
        Note:
            xx method is deprecated, please use :class:`~.core.circuit.Circuit.rxx`.
    ...
    def yy(self, para, obj_qubits, ctrl_qubits=None):
        ...
        Note:
            yy method is deprecated, please use :class:`~.core.circuit.Circuit.ryy`.
    ...
    def zz(self, para, obj_qubits, ctrl_qubits=None):
        ...
        Note:
            zz method is deprecated, please use :class:`~.core.circuit.Circuit.rzz`.
