...
index_t VectorState<qs_policy_t_>::ApplyGate(const std::shared_ptr<BasicGate>& gate,
    ...
    switch (id) {
        ...
        case GateID::M: {
            bool reset = static_cast<MeasureGate*>(gate.get())->reset_;
            return this->ApplyMeasure(gate,reset);
        }break;
        case GateID::PL:
...
template <typename qs_policy_t_>
auto VectorState<qs_policy_t_>::ApplyMeasure(const std::shared_ptr<BasicGate>& gate, bool reset) -> index_t {
    ...
    if (reset)
    {
        qs_policy_t::ConditionalMul(qs, &qs, one_mask, collapse_mask, norm_fact, 0.0, dim, reset);
    }
    else{
...
std::map<std::string, int> VectorState<qs_policy_t_>::ApplyCircuit(const circuit_t& circ,
    ...
    for (auto& g : circ) {
        if (g->id_ == GateID::M) {
            bool reset = static_cast<MeasureGate*>(g.get())->reset_;
            result[static_cast<MeasureGate*>(g.get())->name_] = ApplyMeasure(g, reset);
        } else {
