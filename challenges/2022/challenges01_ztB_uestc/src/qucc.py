...
class QubitUCCAnsatz(Ansatz):
    ...
    def __init__(self, mol, n_qubits=None, n_electrons=None, occ_orb=None, vir_orb=None, generalized=False,th=1e-2, trotter_step=1):
    ...
    def _single_qubit_excitation_circuit(self, i, k, theta):
        ...
        self._circuit += CNOT(i, k)
        self._circuit += RY(theta).on(k, i)
        self._circuit += CNOT(i, k)
    ...
    def _double_qubit_excitation_circuit(self, i, j, k, l, theta):  # noqa: E741 # pylint: disable=too-many-arguments
        ...
        self._circuit += CNOT.on(k, l)
        self._circuit += CNOT.on(i, j)
        self._circuit += CNOT.on(j, l)
        self._circuit += X.on(k)
        self._circuit += X.on(i)
        self._circuit += RY(theta).on(l, ctrl_qubits=[i, j, k])
        self._circuit += X.on(k)
        self._circuit += X.on(i)
        self._circuit += CNOT.on(j, l)
        self._circuit += CNOT.on(i, j)
    ...
    def _implement(
        ...
        for trotter_idx in range(trotter_step):
            ...
            for (p, q) in itertools.product(vir_indices_spin, occ_indices_spin):
                ...
                amps = self.mol.ccsd_single_amps[p, q]
                ...
                if list(q_pq.terms):
                    ...
                    # print(coeff_s.real)
                    self._single_qubit_excitation_circuit(q, p, coeff_s)
                    singles_counter += 1
                    # self.params.append(amps)
            ...
            for pq_counter, (p_idx, q_idx) in enumerate(itertools.product(range(n_vir_spin), range(n_vir_spin))):
                ...
                for rs_counter, (r_idx, s_idx) in enumerate(itertools.product(range(n_occ_spin), range(n_occ_spin))):
                    ...
                    amps = self.mol.ccsd_double_amps[p, s, q, r]
                    ...
                    if list(q_pqrs.terms):
                        ...
                        self._double_qubit_excitation_circuit(r, s, p, q, coeff_d)
                        doubles_counter += 1
                        
        n_qubits_circuit = 0
        self.num_params = (doubles_counter + singles_counter)*trotter_step
        ...
        print(doubles_counter + singles_counter)
