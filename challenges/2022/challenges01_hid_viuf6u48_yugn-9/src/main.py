...
class VQEoptimizer:
    ...
    def generate_circuit(self, molecule=None, seed=1202):
        ...
        self.circuit = Circuit([
            X.on(i)
            for i in np.where(code_col < molecule.n_electrons - 2 * nfrozen)[0]
        ])
...
class Main:
    ...
    def run(self, prefix, molecular_file):
        ...
        mol._pyscf_data[
            'mol'].symmetry_subgroup = 'C1' if 'ch4' in prefix.lower(
            ) else None
        ...
        if check_frozen(mol._pyscf_data['scf'].mo_energy):
            frozen = [
                i
                for i in range(check_frozen(mol._pyscf_data['scf'].mo_energy))
            ]
            en_mp2_frozen = mp2_frozen_core_energy(mol._pyscf_data['mp2'],
                                                   frozen=frozen)
        else:
            en_mp2_frozen = 0.0
