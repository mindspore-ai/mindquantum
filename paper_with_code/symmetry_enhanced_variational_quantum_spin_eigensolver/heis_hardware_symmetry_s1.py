...
def N_block(params_str, wires):
    circuit = Circuit()
    circuit += RZ(-np.pi / 2).on(wires[1])

    circuit += CNOT.on(wires[0], wires[1])

    circuit += RZ({params_str: -2}).on(wires[0])
    circuit += RZ(np.pi / 2).on(wires[0])

    circuit += RY({params_str: 2}).on(wires[1])
...
if __name__ == '__main__':
    ...
    circ = ansatz(prefix, N, num_layer).as_ansatz()
    ham = Heis_Ham(N, J)
