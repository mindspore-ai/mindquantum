...
tictactoe, USAMP, entropy with oracle
@author: jonze
"""
import math
import numpy as np
...
best_performance = np.zeros([NUM_ENSEMBLE,N_label+1])
...
for i in range(NUM_ENSEMBLE):
    ...
    #initialize a model with oracle of three samples randomly selected from the pool, belongiing to three different classes
    i0 = np.random.randint(0,99)
    i1 = i0 + 100
    i2 = i0 + 200
    query_status[i0] = 1
    query_status[i1] = 1
    query_status[i2] = 1
    training_set.append(training_pool[i0])
    training_set.append(training_pool[i1])
    training_set.append(training_pool[i2])
    print(training_set,'is the initial set with oracles')
    ...
    best_performance[i,0] = test_best
    ...
    for j in range(N_label):
        ...
        best_performance[i,j+1] = test_best

with open('EQNN-Z_oracle.pickle', 'wb') as f:
