...
def FSamp(param_opt,pool_x,query_status,Fid):
    ...
    index = 0
    for i in range(len(pool_x)):
        if abs(predict_pool[i][0])+0.1*(1-F_mean[i]) <= min_uncertainty and query_status[i] == 0:
            min_uncertainty = abs(predict_pool[i][0])+0.1*(1-F_mean[i])
            index = i
...
for i in range(NUM_ENSEMBLE):
    ...
    for j in range(N_label):
        ...
        if j == 0:
            ...
            index = FSamp(param_opt,pool_x,query_status,Fid) #USamp for index
            query_status[index] = 1
...
with open('EQNN-Z_F.pickle', 'wb') as f:
