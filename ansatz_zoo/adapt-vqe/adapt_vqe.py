...
from mindquantum import TimeEvolution, Circuit, Simulator
from mindquantum import X
from mindquantum import FermionOperator
from mindquantum import Transform
from q_ham import MolInfoProduce
from pool import singlet_SD
...
class AdaptVqe(MolInfoProduce):
    def __init__(self,
        ...
        super(AdaptVqe, self).__init__(geometry, basis, charge, multiplicity,
                                       fermion_transform)
        self.paras = []
        self.gradient_pqc = None
        self.e_pqc = None
        self.adapt_circuit = Circuit(
            [X.on(i) for i in range(self.n_electrons)])
        self.ferop_seq = []
        self.fermion_pool = fermion_pool
        self.step_energies = []
        self.gradients_seq = []

    def compute_gradients(self, fermion_pool, index, delta=1e-5):
        """
    ...
    def paras_optimize(self):
        ...
        result = minimize(self.adapt_energy,
                          self.paras,
                          args=(self.pqc),
                          method='BFGS',
                          jac=True,
    ...
    def opti_process(self, maxiter=100, adapt_thresh=1e-3):
        for iteration in range(maxiter):
            ...
            print('-----------------current_iteration {}-----------------'.
                  format(iteration))
            print('\n')
            sorted_result = self.compute_gradients(self.fermion_pool,
                                                   iteration)
            ...
            if flag:
                ...
                break
            print('optimized energy {} hartree for iteration {}'.format(
                self.step_energies[-1], iteration))

        print(
            'algorithm convergence because of the norm of all operators {} lowen than adapt_thresh {}'
            .format(norm, adapt_thresh))

        print(
            'If not, the algorithm stops due to the error caused by finite difference when calculating gradients'
        )
        print('\n')
        ...
        print('hf_energy {} hartree and fci energy {} hartree'.format(
            self.hf_energy, self.fci_energy))
        print('absolute error compared with fci energy:')
        print('{} hartree'.format(abs(self.step_energies[-1] -
                                      self.fci_energy)))
...
if __name__ == "__main__":
    ...
    geometry = [['H', (0, 0, 0)], ['H', (0, 0, 2.0)], ['H', (0, 0, 4.0)],
                ['H', (0, 0, 6.0)]]
