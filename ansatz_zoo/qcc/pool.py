...
def check_operator_requirement(ferop):
    if not ferop.terms:
        ...
        return max(
            len(term) for term, coeff in ferop.terms.items()
            if (abs(coeff.const) > 1e-12))
    ...
    def generate_operators(self):
        ...
        for i in range(0, n_occ):
            ...
            for a in range(0, n_vir):
                ...
                if check_operator_requirement(termA) > 0:
                    termA = termA / np.sqrt(coeffA.const)
                    self.fermi_ops.append(termA)
        for i in range(0, n_occ):
            ia = 2 * i
            ib = 2 * i + 1
            ...
            for j in range(i, n_occ):
                ...
                for a in range(0, n_vir):
                    ...
                    for b in range(a, n_vir):
                        ...
                        if check_operator_requirement(termB) > 0:
                            termB = termB / np.sqrt(coeffB.const)
                            self.fermi_ops.append(termB)
...
#                        print()
                        ...
                        termB = FermionOperator(
                            ((ra, 1), (pa, 0), (sb, 1), (qb, 0)), 1 / 2.0)
                        termB += FermionOperator(
                            ((rb, 1), (pb, 0), (sa, 1), (qa, 0)), 1 / 2.0)
                        termB += FermionOperator(
                            ((ra, 1), (pb, 0), (sb, 1), (qa, 0)), -1 / 2.0)
                        termB += FermionOperator(
                            ((rb, 1), (pa, 0), (sa, 1), (qb, 0)), -1 / 2.0)
...
def pauli_pool(collection_of_ferops):
    collection_of_jw_qubitops = [
        Transform(ferop).jordan_wigner() for ferop in collection_of_ferops
    ]
...
def rename_pauli_string(single_pauli_string, iteration):
    ...
    for pauli_seq, coeff in single_pauli_string.terms.items():
        renamed_single_pauli_string = QubitOperator(pauli_seq,
                                                    'p' + str(iteration))
