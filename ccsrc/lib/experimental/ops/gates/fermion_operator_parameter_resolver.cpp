...
auto FermionOperatorPR::matrix(std::optional<uint32_t> n_qubits) const -> std::optional<csr_matrix_t> {
    if (std::empty(terms_)) {
        return std::nullopt;
    }
    ...
    auto result = (process_term(it->first) * it->second.const_value).eval();
    ++it;
    for (; it != end(terms_); ++it) {
        ...
        result = result * process_term(it->first) * it->second.const_value;
    }
...
auto FermionOperatorPR::normal_ordered() const -> FermionOperatorPR {
    FermionOperatorPR ordered_op;
...
auto FermionOperatorPR::normal_ordered_term_(terms_t local_ops, coefficient_t coeff) -> FermionOperatorPR {
    ...
    if (std::empty(local_ops)) {
        return FermionOperatorPR(local_ops, coeff);
    }
    ...
    return ordered_term += FermionOperatorPR(local_ops, coeff);
}
}  // namespace mindquantum::ops
