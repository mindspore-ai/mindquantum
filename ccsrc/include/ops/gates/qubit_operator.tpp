...
auto QubitOperator<coeff_t>::sparse_matrix(std::optional<uint32_t> n_qubits) const -> std::optional<sparse_matrix_t> {
    ...
    const auto process_term = [n_qubits_value, &pauli_matrices](const auto& local_ops, const auto& coeff) -> sparse_matrix_t {
        ...
        return std::accumulate(
            begin(order), end(order), init, [&local_ops, &pauli_matrices](const sparse_matrix_t& init, const auto& idx) {
#ifdef _MSC_VER
                constexpr auto offset = static_cast<std::underlying_type_t<TermValue>>(TermValue::I);
#endif  // _MSC_VER
                if (idx < 0) {
