 ...
 */
#ifndef INCLUDE_VECTOR_DETAIL_CUQUANTUM_VECTOR_FLOAT_POLICY_CUH
#define INCLUDE_VECTOR_DETAIL_CUQUANTUM_VECTOR_FLOAT_POLICY_CUH
...
// Structure defining the cuQuantum vector policy for single-precision floating-point numbers.
// Inherits from GPUVectorPolicyBase to reuse common functionalities.
struct CuQuantumVectorPolicyFloat : public GPUVectorPolicyBase<CuQuantumVectorPolicyFloat, float> {
    using Base = GPUVectorPolicyBase<CuQuantumVectorPolicyFloat, float>;
 ...
 private:
    // Helper for applying 4x4 matrix (float)
    static void apply_two_qubit_matrix(qs_data_p_t* qs_p, const qbits_t& objs, const qbits_t& ctrls,
                                       const std::complex<float>* matrix, index_t dim, const std::string& gate_name);
};

}  // namespace mindquantum::sim::vector::detail

#endif  // INCLUDE_VECTOR_DETAIL_CUQUANTUM_VECTOR_FLOAT_POLICY_CUH
