...
def test_hamiltonian():
    ham = Hamiltonian(QubitOperator('X0 Y3', 0.5)+ 0.6 * QubitOperator('X1 Z2'))
...
def test_grad():
    ...
    circ += XX('a').on([0,1])
    circ += S.on(3)
    circ += PhaseShift({'a':0.2,'b':0.5}).on(1,0)
    circ += X.on(1,0)
    circ += RY('b').on(0)
    circ += XX('a').on([0,1],2)
    circ += H.on(2,1)
    circ += RY('b').on(0,1)
    circ += PhaseShift({'a':0.2,'b':0.5}).on(2)
    circ += RZ(1).on(3)
    circ += RZ('b').on(3)
    circ += RX(1).on(2)
    circ2 = Circuit()
...
def test_noise_grad():
    ...
    # test noise grad
    noise_circ = circ.with_noise(AmplitudeDampingChannel(0.5))

    print(noise_circ.hermitian())
    noise_grad_ops = m_sim.get_expectation_with_grad([ham,ham1], noise_circ)
    nf, ng1, ng2 = noise_grad_ops(np.array([[2,5],[3,0.2]]),np.array([1,2]))
    print(mg1)
    print()
